...
joblib.dump(scaler, f"{output_dir}\\scaler_NN_v2_70_30.pkl")
print("Done.")
...
joblib.dump(le, f"{output_dir}\\label_encoder_NN_v2_70_30.pkl")
...
model.save(f"{output_dir}\\model_NN_v2_70_30.keras")
print("Model saved.")
...
pd.DataFrame({'y_true': y_test_labels, 'y_pred': y_pred}).to_csv(
    f"{output_dir}\\Predictions_NN_v2_70_30.csv", index=False)
...
print("\n========== NEURAL NETWORK RESULTS (v2 70:30) ==========")
...
plt.title('Confusion Matrix - Neural Network v2 (70:30)\n(% of actual class)', fontsize=13)
plt.ylabel('Actual', fontsize=11)
plt.xlabel('Predicted', fontsize=11)
plt.xticks(rotation=45, ha='right', fontsize=8)
plt.yticks(rotation=0, fontsize=8)
plt.tight_layout()
plt.savefig(f"{output_dir}\\ConfusionMatrix_NN_v2_70_30.png", dpi=120, bbox_inches='tight')
plt.show()
...
axes[1].legend()
plt.tight_layout()
plt.savefig(f"{output_dir}\\TrainingCurves_NN_v2_70_30.png", dpi=120, bbox_inches='tight')
plt.show()
...
}).to_csv(f"{output_dir}\\Results_NN_v2_70_30.csv", index=False)
print("Results saved.")
